...
class CheckOverdueBorrowingsTaskTests(TestCase):
    ...
    def setUp(self):
        ...
        self.book1 = Book.objects.create(title="The Great Gatsby", inventory=5, daily_fee=0.50)
        self.book2 = Book.objects.create(title="1984", inventory=3, daily_fee=0.75)

    @contextmanager
    def mock_date_and_notification(self, fixed_date=BASE_DATE):
        with patch("borrowings.tasks.date") as mock_date, \
             patch("borrowings.tasks.send_notification_to_all_admin_users") as mock_send:
            mock_date.today.return_value = fixed_date
            yield mock_date, mock_send

    def create_borrowing(self, book, user, borrow_date, expected_return, actual_return=None):
    ...
    def test_single_overdue_borrowing_sends_notification(self):
        with self.mock_date_and_notification() as (mock_date, mock_send_notification):
            ...
            expected_message = self.get_expected_message(borrowing_overdue, fixed_today)
    ...
    def test_multiple_overdue_borrowings_sends_notifications(self):
        with self.mock_date_and_notification(fixed_date=date(2024, 5, 20)) as (mock_date, mock_send_notification):
            ...
            mock_send_notification.assert_has_calls([call(message1), call(message2)], any_order=True)
    ...
    def test_no_overdue_borrowings_sends_correct_message(self):
        with self.mock_date_and_notification() as (mock_date, mock_send_notification):
            ...
            check_overdue_borrowings()
            mock_send_notification.assert_called_once_with("<b>No borrowings overdue today!</b>")

    def test_borrowing_due_today_is_overdue(self):
        with self.mock_date_and_notification(fixed_date=date(2024, 6, 1)) as (mock_date, mock_send_notification):
            ...
            check_overdue_borrowings()
            expected_message = self.get_expected_message(borrowing, fixed_today)
            mock_send_notification.assert_called_once_with(expected_message)
    ...
    def test_large_number_of_borrowings(self):
        with self.mock_date_and_notification() as (mock_date, mock_send_notification):
            ...
            Borrowing.objects.bulk_create(overdue_borrowings + future_borrowings)
            check_overdue_borrowings()
