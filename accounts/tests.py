...
class UserRegistrationActivationTests(APITestCase):
    ...
    def test_user_registration_sends_activation_email(self):
        ...
        self.assertFalse(user.is_active)  # тепер перевіряємо is_active
    ...
    def test_activation_with_valid_token(self):
        ...
        response = self.client.post(self.activation_url, {"uid": uid, "token": token}, format="json")
        user.refresh_from_db()
        self.assertIn(response.status_code, [status.HTTP_200_OK, status.HTTP_204_NO_CONTENT])
        self.assertTrue(user.is_active)  # тепер перевіряємо is_active

    def test_activation_with_invalid_token(self):
        self.client.post(self.registration_url, self.user_data, format="json")
        user = User.objects.get(email=self.user_data["email"])
        uid = encode_uid(user.pk)
        response = self.client.post(self.activation_url, {"uid": uid, "token": "invalidtoken"}, format="json")
        user.refresh_from_db()
    ...
    def test_login_after_activation(self):
        ...
        uid = encode_uid(user.pk)
        token = default_token_generator.make_token(user)
        self.client.post(self.activation_url, {"uid": uid, "token": token}, format="json")
        response = self.client.post(self.login_url, {
            "email": self.user_data["email"],
            "password": self.user_data["password"]
        }, format="json")
    ...
    def test_registration_with_short_password(self):
        data = {"email": "shortpass@example.com", "password": "123", "re_password": "123"}
        response = self.client.post(self.registration_url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("password", response.data)

    def test_registration_with_no_email(self):
        data = {"password": "ValidPass123!", "re_password": "ValidPass123!"}
        response = self.client.post(self.registration_url, data, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("email", response.data)
    ...
    def test_repeated_activation_link(self):
        ...
        uid = encode_uid(user.pk)
        token = default_token_generator.make_token(user)
        response1 = self.client.post(self.activation_url, {"uid": uid, "token": token}, format="json")
        user.refresh_from_db()
        self.assertIn(response1.status_code, [status.HTTP_200_OK, status.HTTP_204_NO_CONTENT])
        self.assertTrue(user.is_active)
        response2 = self.client.post(self.activation_url, {"uid": uid, "token": token}, format="json")
